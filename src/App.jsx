...
function renderBody(text) {
  ...
  lines.forEach((line, i) => {
    ...
    if (isMedia) {
      out.push(<div key={i} style={{margin:"0.6rem 0",maxWidth:480}}><img src={trimmed} alt="" style={{width:"100%",display:"block",borderRadius:2,border:"1px solid #222"}} loading="lazy"/></div>);
      return;
    }
...
export default function App() {
  ...
  const refreshProfile = () => { if (session) loadProfile(session.user.id); };
  ...
  return (
    <Layout profile={profile} view={view} setView={setView} search={search} setSearch={setSearch} goHome={goHome}>
      ...
      {view==="profile" && <ProfileView profile={profile} onOpenThread={openThread} onProfileUpdate={refreshProfile}/>}
    </Layout>
  );
}
...
function BannedPage() {
  return (
    <div style={{minHeight:"100vh",background:"#000",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",fontFamily:"monospace"}}>
      <div style={{fontSize:"1.2rem",color:"#ef4444",fontWeight:700,marginBottom:8}}>Account Banned</div>
      <div style={{fontSize:12,color:"#555",marginBottom:"1.5rem"}}>Your account has been banned from this forum.</div>
      <button onClick={()=>sb.auth.signOut()} style={{background:"none",border:"1px solid #333",padding:"6px 16px",color:"#555",fontSize:11,fontFamily:"monospace"}}>Sign Out</button>
    </div>
  );
}
...
function LoginPage() {
  ...
  const submit = async () => {
    ...
    try {
      if (mode === "signup") {
        const { error } = await sb.auth.signUp({ email, password: pass, options: { data: { username: uname } } });
    ...
    <div style={{minHeight:"100vh",background:"#000",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",fontFamily:"monospace"}}>
      ...
      <div style={{width:"100%",maxWidth:340,background:"#0a0a0a",border:"1px solid #1a1a1a",padding:"2rem"}}>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:"0.8rem"}}>
          {mode==="signup" && <LabeledInput label="Username" value={uname} onChange={setUname} placeholder="your_handle"/>}
          ...
          {err && <div style={{fontSize:11,color:err.includes("created")||err.includes("Check")?"#22c55e":"#f87171",padding:"6px 8px",background:"#111",border:`1px solid ${err.includes("created")||err.includes("Check")?"#166534":"#7f1d1d"}`}}>{err}</div>}
          <button onClick={submit} disabled={busy} style={{background:"#22c55e",color:"#000",border:"none",padding:"10px",fontSize:12,fontFamily:"monospace",fontWeight:700,letterSpacing:"0.15em",textTransform:"uppercase",cursor:busy?"not-allowed":"pointer",opacity:busy?0.6:1,marginTop:4}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(180px,1fr))",gap:"1px",border:"1px solid #1a1a1a",marginTop:10}}>
          {mainCats.map(cat=>(
            <button key={cat.slug} onClick={()=>onSelectCat(cat.slug)}
              ...
              {cat.min_role!=="member"&&<div style={{fontSize:10,color:ROLES[cat.min_role]?.color||"#555"}}>[{ROLES[cat.min_role]?.label}]</div>}
            </button>
          ))}
        </div>
      </div>
  ...
  return (
    <div onClick={onClick} className="row" style={{display:"grid",gridTemplateColumns:"1fr auto",gap:"0.8rem",alignItems:"center",padding:"0.85rem 1rem",borderBottom:last?"none":"1px solid #1a1a1a",cursor:"pointer",background:"#000",position:"relative",transition:"background 0.1s"}}>
      ...
      <div style={{textAlign:"right",flexShrink:0}}>
        <div style={{fontSize:11,color:"#444"}}>{replyCount} replies</div>
...
function ThreadView({ initThread, profile, onBack }) {
  ...
  const load = useCallback(async () => {
    ...
    const [{data:th},{data:rpl},{data:log}] = await Promise.all([
      sb.from("threads").select("*,categories(slug,label,min_role),profiles(username,role,avatar_url,bio)").eq("id",initThread.id).single(),
  ...
  const logAction = async (action, detail="") => {
    await sb.from("mod_log").insert({thread_id:thread.id, mod_id:profile.id, action, detail});
  };
...
function ReactBar({ type, id, profile }) {
  ...
  return (
    <div style={{display:"flex",gap:4,flexWrap:"wrap"}}>
      {REACTIONS.map(e=>{
        const c=counts[e]; const mine=c?.mine;
        return (
          <button key={e} onClick={()=>react(e)}
            style={{background:mine?"#0f2d1a":"#0a0a0a",border:`1px solid ${mine?"#22c55e55":"#1a1a1a"}`,padding:"2px 8px",fontSize:10,color:mine?"#22c55e":"#444",transition:"all 0.1s",display:"flex",gap:4,alignItems:"center"}}>
...
function NewThreadView({ profile, activeCat, onBack, onCreated }) {
  ...
  useEffect(()=>{ sb.from("categories").select("*").order("sort_order").then(({data})=>setCats(data||[])); },[]);
...
function AdminView({ profile }) {
  ...
  const banUser = async (uid,banned) => {
    await sb.from("profiles").update({banned}).eq("id",uid);
    await sb.from("mod_log").insert({mod_id:profile.id,action:banned?"ban":"unban",detail:`user ${uid}`});
    setUsers(users.map(u=>u.id===uid?{...u,banned}:u));
  };
                ...
                <div key={u.id} style={{display:"grid",gridTemplateColumns:"auto 1fr auto",gap:"0.8rem",alignItems:"center",padding:"0.75rem 1rem",background:u.banned?"#110000":i%2===0?"#0a0a0a":"#000",borderBottom:i===users.length-1?"none":"1px solid #1a1a1a"}}>
                  ...
                  <div>
                    <div style={{fontSize:12,color:u.banned?"#ef4444":ROLES[u.role]?.color||"#555",fontWeight:700}}>
                      {u.username} {u.banned&&<span style={{fontSize:9,color:"#ef4444"}}>[BANNED]</span>}
                    </div>
                  ...
                  <div style={{display:"flex",gap:4,alignItems:"center"}}>
                    {profile.role==="admin"&&u.id!==profile.id&&(
                      <select value={u.role} onChange={e=>setRole(u.id,e.target.value)}
                        style={{background:"#000",border:"1px solid #1a1a1a",padding:"4px 8px",color:ROLES[u.role]?.color||"#555",fontSize:11}}>
...
function ProfileView({ profile, onOpenThread, onProfileUpdate }) {
  ...
  const saveProfile = async () => {
    setSaving(true);
    await sb.from("profiles").update({bio:bio.trim(),avatar_url:avatarUrl}).eq("id",profile.id);
    setSaving(false);
    setEditing(false);
    onProfileUpdate();
  };
            ...
            {editing&&(
              <>
                <button onClick={()=>fileRef.current.click()}
                  style={{position:"absolute",bottom:-6,right:-6,background:"#22c55e",color:"#000",border:"none",borderRadius:2,fontSize:9,padding:"2px 5px",fontFamily:"monospace",fontWeight:700}}>
                  {uploading?"...":"edit"}
                </button>
