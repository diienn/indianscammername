...
const CATEGORIES = [
  { slug: "all",           label: "All",           min_role: "member" },
  { slug: "announcements", label: "Announcements", min_role: "moderator" },
  { slug: "guides",        label: "Guides",        min_role: "member" },
  { slug: "youtube",       label: "YouTube",       min_role: "moderator" },
  { slug: "showcase",      label: "Showcases",     min_role: "member" },
  { slug: "mods",          label: "Mods",          min_role: "member" },
  { slug: "templates",     label: "Templates",     min_role: "member" },
  { slug: "help",          label: "Help",          min_role: "member" },
  { slug: "offtopic",      label: "Off-Topic",     min_role: "member" },
];

const ALL_TAGS = [
  "bnm","il2cpp","unity","dobby","openxr","quest","vr","vulkan",
  "ndk","cmake","imgui","egl","frida","ghidra","metadata","arm64",
  "hook","android","beginner","template","guide","crash","help","build",
];
...
function renderBody(text) {
  ...
  lines.forEach((line, i) => {
    if (line.startsWith("```")) {
      if (inCode) {
        out.push(
          <pre key={i} style={{ background: "#0a0a0a", border: "1px solid #222", borderRadius: 2, padding: "0.8rem 1rem", overflowX: "auto", fontFamily: "monospace", fontSize: 12, color: "#22c55e", margin: "0.5rem 0", lineHeight: 1.6 }}>
            {codeLines.join("\n")}
          </pre>
        );
    ...
    const isMedia = /^https?:\/\/\S+\.(gif|png|jpg|jpeg|webp)(\?\S*)?$/i.test(trimmed) ||
      /^https?:\/\/(media\.tenor\.com|media\.giphy\.com|i\.imgur\.com)\S*$/i.test(trimmed);

    if (isMedia) {
      out.push(
        <div key={i} style={{ margin: "0.6rem 0", maxWidth: 480 }}>
          <img src={trimmed} alt="" style={{ width: "100%", display: "block", borderRadius: 2, border: "1px solid #222" }} loading="lazy" />
        </div>
      );
      return;
    }
    ...
    if (line.startsWith("# "))       out.push(<h2 key={i} style={{ fontSize: "1.2rem", color: "#fff", margin: "0.8rem 0 0.3rem", fontWeight: 700 }}>{line.slice(2)}</h2>);
      ...
      out.push(
        <p key={i} style={{ color: "#999", marginBottom: 2, lineHeight: 1.7, fontSize: 13.5 }}>
          {parts.map((p, j) =>
            p.startsWith("**") && p.endsWith("**")
              ? <strong key={j} style={{ color: "#ccc", fontWeight: 600 }}>{p.slice(2, -2)}</strong>
              : p
          )}
        </p>
      );
...
export default function App() {
  ...
  if (!session) return <LoginPage onAuth={() => {}} />;

  return (
    <Layout
      profile={profile} view={view} setView={setView}
      search={search} setSearch={setSearch} goHome={goHome}
    >
      ...
      {view === "profile" && <ProfileView profile={profile} onOpenThread={openThread} />}
    </Layout>
  );
}
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#000", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", fontFamily: "monospace" }}>
      ...
      <div style={{ marginBottom: "2.5rem", textAlign: "center" }}>
        <div style={{ fontSize: "1.4rem", fontWeight: 700, color: "#fff", letterSpacing: "0.05em", marginBottom: 6 }}>
          LearnAndroidHacking
        </div>
        <div style={{ fontSize: 11, color: "#444", letterSpacing: "0.15em", textTransform: "uppercase" }}>
          Free Android Modding Community
        </div>
      </div>

      <div style={{ width: "100%", maxWidth: 360, background: "#0a0a0a", border: "1px solid #222", padding: "2rem" }}>
        <div style={{ display: "flex", borderBottom: "1px solid #222", marginBottom: "1.5rem" }}>
          {["login", "signup"].map(m => (
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "0.8rem" }}>
          {mode === "signup" && (
            <LabeledInput label="Username" value={uname} onChange={setUname} placeholder="your_handle" />
          )}
          ...
          {err && (
            <div style={{ fontSize: 11, color: err.startsWith("Check") ? "#22c55e" : "#f87171", fontFamily: "monospace", padding: "6px 8px", background: "#111", border: `1px solid ${err.startsWith("Check") ? "#166534" : "#7f1d1d"}` }}>
              {err}
            </div>
          )}
    ...
    <div style={{ minHeight: "100vh", background: "#000", color: "#ccc", fontFamily: "monospace" }}>
      <style>{`
        *{box-sizing:border-box;margin:0;padding:0}
        ...
        button{cursor:pointer} .row:hover{background:#0a0a0a !important}
      `}</style>
      ...
      <nav style={{ background: "#000", borderBottom: "1px solid #1a1a1a", position: "sticky", top: 0, zIndex: 100 }}>
        <div style={{ maxWidth: 1200, margin: "0 auto", padding: "0 1.2rem", height: 48, display: "flex", alignItems: "center", gap: "1rem" }}>
          <button onClick={goHome} style={{ background: "none", border: "none", fontSize: "0.95rem", fontWeight: 700, color: "#fff", letterSpacing: "0.04em", fontFamily: "monospace" }}>
            LAH
          </button>
          <span style={{ color: "#222", fontSize: 18 }}>|</span>
          <input value={search} onChange={e => setSearch(e.target.value)}
            onFocus={() => setView("forum")}
            placeholder="search..."
            ...
            <div style={{ position: "relative" }}>
              ...
              {menuOpen && (
                <div onClick={() => setMenuOpen(false)}
                  style={{ position: "absolute", right: 0, top: "calc(100% + 2px)", background: "#0a0a0a", border: "1px solid #1a1a1a", minWidth: 140, zIndex: 200 }} className="fi">
      ...
      <div style={{ maxWidth: 1200, margin: "0 auto", padding: "1.5rem 1.2rem" }}>
        {children}
      </div>
    </div>
  );
...
function NavBtn({ label, active, onClick }) {
  return (
    <button onClick={onClick}
      style={{ background: "none", border: "none", fontSize: 11, color: active ? "#fff" : "#555", fontFamily: "monospace", letterSpacing: "0.08em", textTransform: "uppercase", padding: "4px 0", borderBottom: `1px solid ${active ? "#22c55e" : "transparent"}` }}>
      {label}
    </button>
  );
}
function DDBtn({ label, onClick, dim }) {
  return (
    <button onClick={onClick} className="row"
      style={{ display: "block", width: "100%", background: "none", border: "none", padding: "8px 1rem", fontSize: 11, color: dim ? "#555" : "#ccc", fontFamily: "monospace", textAlign: "left" }}>
      {label}
    </button>
  );
}
function Dot({ role }) {
  const c = ROLES[role]?.color || "#555";
  ...
  return (
    <div className="fi">
      ...
      <div style={{ paddingBottom: "2rem", marginBottom: "2rem", borderBottom: "1px solid #1a1a1a" }}>
        <div style={{ fontSize: 11, color: "#333", letterSpacing: "0.15em", textTransform: "uppercase", marginBottom: 8 }}>
          Free Android Modding Community
        </div>
        <h1 style={{ fontSize: "1.8rem", color: "#fff", fontWeight: 700, marginBottom: "0.5rem", letterSpacing: "0.02em" }}>
          LearnAndroidHacking
        </h1>
        <p style={{ fontSize: 13, color: "#555", lineHeight: 1.7, maxWidth: 480 }}>
          IL2CPP, native hooking, VR modding, reverse engineering. All free. No paywalls.
        </p>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(180px, 1fr))", gap: "1px", border: "1px solid #1a1a1a", marginTop: 10 }}>
          {mainCats.map(cat => (
            <button key={cat.slug} onClick={() => onSelectCat(cat.slug)}
              ...
              <div style={{ fontSize: 12, color: "#fff", fontWeight: 700, marginBottom: 4, fontFamily: "monospace" }}>{cat.label}</div>
              {cat.min_role !== "member" && (
                <div style={{ fontSize: 10, color: ROLES[cat.min_role]?.color || "#555" }}>
                  [{ROLES[cat.min_role]?.label}]
                </div>
              )}
            </button>
          ))}
        ...
        <div style={{ marginTop: 10, display: "flex", flexDirection: "column", gap: 1 }}>
          {latest.map(t => (
            <div key={t.id} style={{ background: "#0a0a0a", padding: "0.7rem 1rem", display: "flex", justifyContent: "space-between", alignItems: "center", gap: "1rem", flexWrap: "wrap" }}>
              <div>
                <div style={{ fontSize: 13, color: "#ddd", fontFamily: "monospace", marginBottom: 3 }}>{t.title}</div>
...
function ForumView({ profile, activeCat, setActiveCat, activeTag, setActiveTag, search, onOpenThread, onNew }) {
  ...
  const load = useCallback(async () => {
    ...
    let q = sb.from("threads")
      .select("id,title,tags,created_at,views,pinned,locked,category_id,categories(id,slug,label),profiles(username,role),replies(count)")
    ...
    <div style={{ display: "flex", gap: "1.5rem" }} className="fi">
      ...
      <aside style={{ width: 160, flexShrink: 0 }}>
        ...
        <div style={{ marginTop: 8, display: "flex", flexDirection: "column", gap: 1 }}>
          {CATEGORIES.map(c => (
            <button key={c.slug} onClick={() => { setActiveCat(c.slug); setActiveTag(null); }}
        ...
        <div style={{ marginTop: "1.5rem" }}>
          ...
          <div style={{ marginTop: 8, display: "flex", flexWrap: "wrap", gap: 3 }}>
            {ALL_TAGS.map(t => (
              <TagChip key={t} t={t} active={activeTag === t} onClick={() => setActiveTag(activeTag === t ? null : t)} />
            ))}
          </div>
        </div>
      </aside>
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "1rem", gap: 8, flexWrap: "wrap" }}>
          <div>
            <h2 style={{ fontSize: "1rem", color: "#fff", fontWeight: 700, fontFamily: "monospace" }}>
              {activeTag ? `#${activeTag}` : search ? `"${search}"` : CATEGORIES.find(c => c.slug === activeCat)?.label || "Forum"}
            </h2>
            {activeTag && (
              <button onClick={() => setActiveTag(null)} style={{ background: "none", border: "none", fontSize: 10, color: "#555", fontFamily: "monospace", marginTop: 2 }}>
                clear tag
              </button>
            )}
          </div>
          <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
            <select value={sort} onChange={e => setSort(e.target.value)}
              style={{ background: "#0a0a0a", border: "1px solid #1a1a1a", color: "#555", padding: "4px 8px", fontSize: 11, fontFamily: "monospace" }}>
              <option value="latest">Latest</option>
              <option value="views">Most Viewed</option>
            </select>
            <button onClick={onNew}
              style={{ background: "#22c55e", color: "#000", border: "none", padding: "5px 14px", fontSize: 11, fontFamily: "monospace", fontWeight: 700, letterSpacing: "0.1em", textTransform: "uppercase" }}>
              New Thread
            </button>
          </div>
        </div>
    ...
    <div onClick={onClick} className="row"
      ...
      <div style={{ minWidth: 0, paddingLeft: t.pinned ? 8 : 0 }}>
        <div style={{ display: "flex", gap: 6, marginBottom: 3, alignItems: "center", flexWrap: "wrap" }}>
          {t.pinned && <Pill label="PINNED" />}
          {t.locked && <Pill label="LOCKED" dim />}
          {t.categories && <Pill label={t.categories.label} />}
        </div>
        <div style={{ fontSize: 13, color: "#ddd", fontFamily: "monospace", fontWeight: 600, marginBottom: 4, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{t.title}</div>
        <div style={{ display: "flex", alignItems: "center", gap: "0.6rem", flexWrap: "wrap" }}>
          ...
          <div style={{ display: "flex", gap: 3, flexWrap: "wrap" }}>
            {(t.tags || []).slice(0, 4).map(tg => <TagChip key={tg} t={tg} />)}
          </div>
        </div>
      </div>
      <div style={{ textAlign: "right", flexShrink: 0 }}>
        <div style={{ fontSize: 11, color: "#444", fontFamily: "monospace" }}>{replyCount} replies</div>
...
function ThreadView({ initThread, profile, onBack }) {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    const [{ data: th }, { data: rpl }] = await Promise.all([
      sb.from("threads").select("*,categories(slug,label,min_role),profiles(username,role,bio)").eq("id", initThread.id).single(),
      sb.from("replies").select("*,profiles(username,role)").eq("thread_id", initThread.id).order("created_at", { ascending: true }),
    ]);
    setThread(th);
    setReplies(rpl || []);
  ...
  const postReply = async () => {
    ...
    setBusy(true);
    await sb.from("replies").insert({ thread_id: thread.id, author_id: profile.id, body: body.trim() });
    setBody("");
    await load();
    setBusy(false);
  };

  const togglePin  = async () => { await sb.from("threads").update({ pinned: !thread.pinned }).eq("id", thread.id); load(); };
  const toggleLock = async () => { await sb.from("threads").update({ locked: !thread.locked }).eq("id", thread.id); load(); };
  const del        = async () => { if (window.confirm("Delete thread?")) { await sb.from("threads").delete().eq("id", thread.id); onBack(); } };
  const delReply   = async (rid) => { await sb.from("replies").delete().eq("id", rid); load(); };
  ...
  return (
    <div className="fi">
      <button onClick={onBack} style={{ background: "none", border: "none", fontSize: 11, color: "#555", fontFamily: "monospace", marginBottom: "1rem" }}>
        back to forum
      </button>
      ...
      <div style={{ background: "#0a0a0a", border: "1px solid #1a1a1a" }}>
        <div style={{ padding: "1.2rem 1.4rem", borderBottom: "1px solid #1a1a1a" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: "0.8rem" }}>
            <div>
              ...
              <h1 style={{ fontSize: "1.2rem", color: "#fff", fontFamily: "monospace", fontWeight: 700, lineHeight: 1.3 }}>{thread.title}</h1>
            </div>
            {isMod && (
        ...
        <div style={{ padding: "1.2rem 1.4rem", borderBottom: "1px solid #1a1a1a", display: "flex", gap: "0.8rem", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap" }}>
          <div style={{ display: "flex", gap: "0.8rem", alignItems: "center" }}>
            <Avi profile={thread.profiles} />
            <div>
              <div style={{ fontSize: 12, color: ROLES[thread.profiles?.role]?.color || "#555", fontFamily: "monospace", fontWeight: 700 }}>{thread.profiles?.username}</div>
      ...
      {replies.map((r, i) => (
        <div key={r.id} style={{ background: i % 2 === 0 ? "#000" : "#050505", border: "1px solid #1a1a1a", borderTop: "none" }}>
          <div style={{ padding: "0.8rem 1.4rem", display: "flex", gap: "0.8rem", alignItems: "flex-start" }}>
            <Avi profile={r.profiles} />
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <div style={{ marginTop: 6 }}>
                <ReactBar type="reply" id={r.id} profile={profile} />
              </div>
            </div>
          </div>
        </div>
        ...
        {thread.locked ? (
          ...
          <>
            <div style={{ fontSize: 10, color: "#333", marginBottom: 6, letterSpacing: "0.1em", textTransform: "uppercase" }}>
              ...
              <span style={{ marginLeft: 8, color: "#222" }}>— paste a GIF URL on its own line to embed it</span>
            </div>
            ...
            <div style={{ display: "flex", justifyContent: "flex-end", marginTop: 8 }}>
              <button onClick={postReply} disabled={!body.trim() || busy}
                style={{ background: body.trim() && !busy ? "#22c55e" : "#111", color: body.trim() && !busy ? "#000" : "#333", border: "none", padding: "6px 18px", fontSize: 11, fontFamily: "monospace", fontWeight: 700, letterSpacing: "0.1em", textTransform: "uppercase" }}>
                {busy ? "..." : "Post Reply"}
              </button>
            </div>
          </>
      ...
      {REACTIONS.map(e => {
        ...
        return (
          <button key={e} onClick={() => react(e)}
            style={{ background: mine ? "#0f2d1a" : "#0a0a0a", border: `1px solid ${mine ? "#22c55e55" : "#1a1a1a"}`, padding: "2px 8px", fontSize: 10, color: mine ? "#22c55e" : "#444", fontFamily: "monospace", transition: "all 0.1s", display: "flex", gap: 4, alignItems: "center" }}>
            {REACTION_DISPLAY[e]}{c?.count ? <span style={{ color: mine ? "#22c55e" : "#333" }}>{c.count}</span> : null}
          </button>
        );
      })}
    </div>
...
function NewThreadView({ profile, activeCat, onBack, onCreated }) {
  ...
  const [body, setBody]     = useState("");
  const [catSlug, setCat]   = useState(activeCat !== "all" ? activeCat : "help");
  ...
  useEffect(() => {
    sb.from("categories").select("*").order("sort_order").then(({ data }) => setCats(data || []));
  }, []);
  ...
  const onTagKey = e => {
    ...
    if (v.endsWith(" ") || v.endsWith(",")) {
      const c = v.trim().replace(/,/g, "").toLowerCase().replace(/[^a-z0-9-]/g, "");
      if (c) addTag(c);
      setTI("");
    } else setTI(v);
  };

  const allowed = cats.filter(c => canPostIn(c, profile?.role));

  const submit = async () => {
    if (!title.trim() || !body.trim()) return;
    setErr(""); setBusy(true);
    const catRow = cats.find(c => c.slug === catSlug);
    if (!catRow || !canPostIn(catRow, profile?.role)) { setErr("Insufficient permissions."); setBusy(false); return; }
  ...
  return (
    <div className="fi" style={{ maxWidth: 760 }}>
      ...
      <h2 style={{ fontSize: "1rem", color: "#fff", fontFamily: "monospace", fontWeight: 700, marginBottom: "1.2rem" }}>New Thread</h2>
      ...
      <div style={{ background: "#0a0a0a", border: "1px solid #1a1a1a", padding: "1.4rem", display: "flex", flexDirection: "column", gap: "1rem" }}>
        ...
        <FRow label="Category">
          <select value={catSlug} onChange={e => setCat(e.target.value)}
            ...
            {allowed.map(c => <option key={c.slug} value={c.slug}>{c.label}</option>)}
          </select>
        </FRow>

        <FRow label="Body (markdown supported — paste a GIF URL on its own line to embed)">
          ...
          <div style={{ background: "#000", border: "1px solid #1a1a1a", padding: "5px 7px", display: "flex", flexWrap: "wrap", gap: 4, minHeight: 34, alignItems: "center", marginBottom: 6 }}>
            {tags.map(t => (
              <span key={t} style={{ fontSize: 10, padding: "2px 7px", background: "#0f2d1a", color: "#22c55e", border: "1px solid #166534", display: "flex", alignItems: "center", gap: 4 }}>
                #{t}
                <button onClick={() => rmTag(t)} style={{ background: "none", border: "none", color: "#22c55e", fontSize: 11, lineHeight: 1, padding: 0 }}>x</button>
              </span>
            ))}
            <input value={tagInput} onChange={onTagKey} placeholder={tags.length === 0 ? "add tags (space to confirm)" : ""}
        ...
        <div style={{ display: "flex", justifyContent: "flex-end", gap: 8 }}>
          <button onClick={onBack} style={{ background: "none", border: "1px solid #1a1a1a", padding: "6px 14px", fontSize: 11, color: "#555", fontFamily: "monospace" }}>Cancel</button>
          <button onClick={submit} disabled={!title.trim() || !body.trim() || busy}
            style={{ background: title.trim() && body.trim() ? "#22c55e" : "#111", color: title.trim() && body.trim() ? "#000" : "#333", border: "none", padding: "6px 18px", fontSize: 11, fontFamily: "monospace", fontWeight: 700, letterSpacing: "0.1em", textTransform: "uppercase" }}>
            {busy ? "..." : "Post Thread"}
          </button>
        </div>
      </div>
...
function AdminView({ profile }) {
  ...
  return (
    <div className="fi">
      <h2 style={{ fontSize: "1rem", color: "#fff", fontFamily: "monospace", fontWeight: 700, marginBottom: "1.2rem" }}>Mod Panel</h2>
            ...
            <div style={{ border: "1px solid #1a1a1a" }}>
              {threads.map((t, i) => (
                <div key={t.id} style={{ display: "grid", gridTemplateColumns: "1fr auto", gap: "0.8rem", alignItems: "center", padding: "0.7rem 1rem", background: i % 2 === 0 ? "#0a0a0a" : "#000", borderBottom: i === threads.length - 1 ? "none" : "1px solid #1a1a1a" }}>
                  <div>
                    <div style={{ fontSize: 12, color: "#ddd", fontFamily: "monospace", marginBottom: 2 }}>{t.title}</div>
            ...
            <div style={{ border: "1px solid #1a1a1a" }}>
              {users.map((u, i) => (
                <div key={u.id} style={{ display: "grid", gridTemplateColumns: "auto 1fr auto", gap: "0.8rem", alignItems: "center", padding: "0.75rem 1rem", background: i % 2 === 0 ? "#0a0a0a" : "#000", borderBottom: i === users.length - 1 ? "none" : "1px solid #1a1a1a" }}>
                  <Avi profile={u} />
                  <div>
                    <div style={{ fontSize: 12, color: ROLES[u.role]?.color || "#555", fontFamily: "monospace", fontWeight: 700 }}>{u.username}</div>
                  ...
                  {profile.role === "admin" && u.id !== profile.id ? (
                    <select value={u.role} onChange={e => setRole(u.id, e.target.value)}
                      style={{ background: "#000", border: "1px solid #1a1a1a", padding: "4px 8px", color: ROLES[u.role]?.color || "#555", fontSize: 11, fontFamily: "monospace" }}>
                      <option value="member">Member</option>
                      <option value="moderator">Moderator</option>
                      <option value="admin">Admin</option>
                    </select>
                  ) : (
                    <span style={{ fontSize: 10, color: ROLES[u.role]?.color || "#555" }}>[{ROLES[u.role]?.label}]</span>
                  )}
                </div>
              ))}
            </div>
...
// ── PROFILE ────────────────────────────────────────────────
function ProfileView({ profile, onOpenThread }) {
  const [threads, setThreads] = useState([]);

  useEffect(() => {
    sb.from("threads")
      .select("id,title,tags,created_at,views,pinned,locked,categories(slug,label),profiles(username,role),replies(count)")
    ...
    <div className="fi">
      <div style={{ background: "#0a0a0a", border: "1px solid #1a1a1a", padding: "1.4rem", marginBottom: "1.5rem" }}>
        <div style={{ display: "flex", gap: "1rem", alignItems: "center", flexWrap: "wrap" }}>
          <Avi profile={profile} size={48} />
          <div>
            <div style={{ fontSize: "1.1rem", color: ROLES[profile.role]?.color || "#fff", fontFamily: "monospace", fontWeight: 700 }}>{profile.username}</div>
            <div style={{ fontSize: 10, color: ROLES[profile.role]?.color || "#555", marginTop: 2 }}>[{ROLES[profile.role]?.label}]</div>
            {profile.bio && <div style={{ fontSize: 12, color: "#555", marginTop: 6 }}>{profile.bio}</div>}
          </div>
          <div style={{ marginLeft: "auto", display: "flex", gap: "1.5rem" }}>
            {[["Posts", profile.post_count], ["Since", new Date(profile.created_at).getFullYear()]].map(([l, v]) => (
              <div key={l} style={{ textAlign: "right" }}>
                <div style={{ fontSize: "1rem", color: "#22c55e", fontFamily: "monospace", fontWeight: 700 }}>{v}</div>
...
function Avi({ profile, size = 28 }) {
  ...
  if (profile?.avatar_url) return <img src={profile.avatar_url} style={{ width: size, height: size, borderRadius: 2, objectFit: "cover", border: `1px solid ${c}33`, flexShrink: 0 }} />;
  return <div style={{ width: size, height: size, borderRadius: 2, background: "#0a0a0a", border: `1px solid ${c}44`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: size * 0.42, color: c, fontWeight: 700, flexShrink: 0, fontFamily: "monospace" }}>{letter}</div>;
}

function Pill({ label, green, dim }) {
  const color = green ? "#22c55e" : dim ? "#555" : "#444";
  return <span style={{ fontSize: 9, padding: "1px 5px", background: `${color}14`, color, border: `1px solid ${color}33`, fontFamily: "monospace", letterSpacing: "0.08em" }}>{label}</span>;
}

function TagChip({ t, active, onClick }) {
  return <span onClick={onClick} style={{ fontSize: 10, padding: "1px 6px", background: active ? "#0f2d1a" : "#0a0a0a", color: active ? "#22c55e" : "#444", border: `1px solid ${active ? "#166534" : "#1a1a1a"}`, cursor: onClick ? "pointer" : "default", fontFamily: "monospace", userSelect: "none", transition: "all 0.1s" }}>#{t}</span>;
}

function MBtn({ label, onClick, red }) {
  const c = red ? "#ef4444" : "#555";
  return <button onClick={onClick} style={{ background: "none", border: `1px solid ${c}33`, padding: "2px 8px", fontSize: 10, color: c, fontFamily: "monospace", letterSpacing: "0.05em" }}>{label}</button>;
}
